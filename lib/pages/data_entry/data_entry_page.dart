...
import 'package:mcqs_entry/common/network/network_checker.dart';
...
class _DataEntryPageState extends State<DataEntryPage> {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          dataEntryProvider.type == "MCQS" ? dataEntryProvider.selectedSubject != "No" ? Expanded(
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Container(
                    margin: EdgeInsets.only(top: 20.h,right: 10.w,left: 10.w),
                    child: const Text('Subject',style: TextStyle(fontWeight: FontWeight.bold),)),
                const Expanded(child:  SubjectsDropDown())
              ],
            ),
          ) : const SizedBox.shrink() : const SizedBox.shrink(),
          dataEntryProvider.selectedTopic != "No" ? Expanded(
            child: Row(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Container(
                    margin: EdgeInsets.only(top: 20.h,right: 10.w,left: 10.w),
                    child: const Text('Topics',style: TextStyle(fontWeight: FontWeight.bold),)),
      ...
      child: Form(
        child: Column(
          ...
          children: [
            ...
            const Text("Enter Question:",style: TextStyle(fontWeight: FontWeight.bold),),
            Container(
            ...
            buildOptionRow(option1, 1, dataEntryProvider,dataEntryProvider.option1),
            buildOptionRow(option2, 2, dataEntryProvider,dataEntryProvider.option2),
            buildOptionRow(option3, 3, dataEntryProvider,dataEntryProvider.option3),
            buildOptionRow(option4, 4, dataEntryProvider,dataEntryProvider.option4),
          ],
        ),
      ),
    );
    ...
    await showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            content: Row(
              children: [
                Image.asset('assets/images/alert.png',height: 50.h,width: 50.w,),
 ...
 buildOptionRow(TextEditingController controller,int optionNumber,DataEntryProvider dataEntryProvider,bool isEnable){
  return Container(
    margin: EdgeInsets.only(top: 10.h),
    decoration: BoxDecoration(
        color: AppColors.greyColor,
    ...
    child: Row(
      children: [
        ...
        CupertinoSwitch(value: isEnable, onChanged: (value){
